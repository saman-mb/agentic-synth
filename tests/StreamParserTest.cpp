...
TEST_CASE("StreamParser: single scalar field fires callback once") {
    ...
    int calls = 0;
    PatchStruct last{};
    sp.setCallback([&](const PatchStruct& p) { ++calls; last = p; });
...
TEST_CASE("StreamParser: osc array updates oscillator params") {
    ...
    PatchStruct last{};
    sp.setCallback([&](const PatchStruct& p) { last = p; });
    sp.feedChunk(R"({"osc": [{"type": 2, "volume": 0.9, "enabled": 1, "detune_cents": 0.0, "fm_ratio": 1.0, "fm_depth": 0.0, "pan": 0.0, "pulse_width": 0.5, "semitone_offset": 0.0, "wavetable_pos": 0.0}]})");
