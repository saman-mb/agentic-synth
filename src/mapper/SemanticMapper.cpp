    ...
    for (const auto& tok : tokens) {
        ...
        if (!customEntries_.empty()) {
            ...
            for (const auto& ce : customEntries_) {
                ...
                if (sc < cfg_.similarity_threshold) continue;
                if (ce.context == ctx && sc > best_ctx_sc) { best_ctx_sc = sc; best_ctx = &ce; }
                else if (ce.context == SoundContext::Generic && sc > best_gen_sc) { best_gen_sc = sc; best_gen = &ce; }
            }

            const CustomEntry* chosen = best_ctx ? best_ctx : best_gen;
            if (chosen) { apply_delta(patch, chosen->delta); ++matched; continue; }
        }
...
const char* contextToStr(SoundContext c) {
    switch (c) {
        case SoundContext::Bass:       return "Bass";
        case SoundContext::Pad:        return "Pad";
        case SoundContext::Lead:       return "Lead";
        case SoundContext::Keys:       return "Keys";
        case SoundContext::Percussion: return "Percussion";
        case SoundContext::Arp:        return "Arp";
        case SoundContext::Texture:    return "Texture";
        default:                       return "Generic";
    }
}

SoundContext contextFromStr(const std::string& s) {
    if (s == "Bass")       return SoundContext::Bass;
    if (s == "Pad")        return SoundContext::Pad;
    if (s == "Lead")       return SoundContext::Lead;
    if (s == "Keys")       return SoundContext::Keys;
    if (s == "Percussion") return SoundContext::Percussion;
    if (s == "Arp")        return SoundContext::Arp;
    if (s == "Texture")    return SoundContext::Texture;
    return SoundContext::Generic;
}
...
OscType oscTypeFromStr(const std::string& s) {
    if (s == "Sawtooth") return OscType::Sawtooth;
    if (s == "Square")   return OscType::Square;
    if (s == "Triangle") return OscType::Triangle;
    if (s == "Noise")    return OscType::Noise;
    if (s == "FM")       return OscType::FM;
    return OscType::Sine;
}

const char* lfoTargetToStr(LfoTarget t) {
    if (t == LfoTarget::Amplitude) return "Amplitude";
    if (t == LfoTarget::Pitch)     return "Pitch";
    return "FilterCutoff";
}

LfoTarget lfoTargetFromStr(const std::string& s) {
    if (s == "Amplitude") return LfoTarget::Amplitude;
...
#undef EMIT_F
    ...
    if (d.lfo0_waveform) {
        if (!first) ss << ','; first = false;
        ss << "\"lfo0_waveform\":\""
           << ((*d.lfo0_waveform == LfoWaveform::Square) ? "Square" : "Sine") << '"';
    }
    if (d.lfo0_target) {
        if (!first) ss << ','; first = false;
...
PatchDelta parseDeltaFromJson(const std::string& json) {
    ...
    auto getF = [&](const char* k) -> std::optional<float> {
        if (!mapHasKey(json, k)) return std::nullopt;
    ...
    if (!ost.empty()) d.osc0_type = oscTypeFromStr(ost);

    if (mapHasKey(json, "filter_type")) d.filter_type = FilterType::LowPass;
    ...
    if (!lt.empty()) d.lfo0_target = lfoTargetFromStr(lt);
    ...
    if (mapHasKey(json, "voice_count")) {
        ...
        p = json.find(':', p);
        try { d.voice_count = static_cast<uint8_t>(std::stoi(json.substr(p + 1))); }
        catch (...) {}
    }
...
void SemanticMapper::loadCustomEntries(const std::string& json_path) {
    ...
    while (i < json.size()) {
        ...
        if (entry_str.find("\"readonly\":true") != std::string::npos) { i = j; continue; }
        ...
        if (delta_start != std::string::npos) {
            ...
            if (brace != std::string::npos) {
                ...
                int dd = 1; size_t k = brace + 1;
                while (k < entry_str.size() && dd > 0) {
                    if (entry_str[k] == '{') ++dd;
                    else if (entry_str[k] == '}') --dd;
                    ++k;
                }
                if (dd == 0)
        ...
        if (!e.keyword.empty()) customEntries_.push_back(std::move(e));
        i = j;
    }
}

void SemanticMapper::parseAndSaveCustomEntries(const std::string& json,
    ...
    if (bracket_end == std::string::npos || bracket_end <= bracket) return;
...
void SemanticMapper::addCustomEntry(CustomEntry e) {
