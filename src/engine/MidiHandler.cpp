...
void MidiHandler::process(const RawMidiMsg& msg) noexcept {
    ...
    switch (type) {
        case 0x80u: handleNoteOff(msg.data1); break;
        case 0x90u:
            if (msg.data2 == 0)
            ...
            break;
        case 0xB0u: handleCC(msg.data1, msg.data2); break;
        default: break;
    }
}

void MidiHandler::setHostTempo(double bpm) noexcept {
    vm_.setHostTempo(bpm);
}

// ── private ───────────────────────────────────────────────────────────────────

void MidiHandler::handleNoteOn(int note, float velocity) noexcept {
    vm_.noteOn(note, velocity);
}

void MidiHandler::handleNoteOff(int note) noexcept {
    vm_.noteOff(note);
}
