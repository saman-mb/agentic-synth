...
inline juce::MemoryBlock makeMessage(IpcMsgType type,
                                     uint32_t instanceId,
    ...
    if (!payload.empty())
        block.copyFrom(payload.data(), static_cast<int>(sizeof(IpcMsgHeader)),
                       static_cast<int>(payload.size()));
    return block;
}

// Parse a received MemoryBlock; returns false when the block is malformed.
inline bool parseMessage(const juce::MemoryBlock& block,
                         IpcMsgHeader& hdrOut,
