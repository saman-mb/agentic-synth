...
void StreamParser::applyField(const std::string& key, const std::string& val) {
    if (key == "master_gain") {
        ...
        if (std::sscanf(val.c_str(), "%f", &v) == 1)
            partial_.master_gain = std::clamp(v, 0.0f, 1.0f);
    } else if (key == "portamento_s") {
        float v;
        if (std::sscanf(val.c_str(), "%f", &v) == 1)
            partial_.portamento_s = std::clamp(v, 0.0f, 5.0f);
    } else if (key == "voice_count") {
        int v;
        if (std::sscanf(val.c_str(), "%d", &v) == 1)
            partial_.voice_count = static_cast<uint8_t>(std::clamp(v, 1, 16));
        ...
        partial_.filter.cutoff_hz =
            std::clamp(extractFloat(val, "cutoff_hz", partial_.filter.cutoff_hz), 20.0f, 20000.0f);
        ...
        partial_.filter_env.attack_s =
            std::clamp(extractFloat(val, "attack_s", partial_.filter_env.attack_s), 0.0f, 10.0f);
        partial_.filter_env.decay_s =
            std::clamp(extractFloat(val, "decay_s", partial_.filter_env.decay_s), 0.0f, 10.0f);
        ...
        partial_.filter_env.release_s =
            std::clamp(extractFloat(val, "release_s", partial_.filter_env.release_s), 0.0f, 20.0f);
    } else if (key == "amp_env") {
        ...
        partial_.amp_env.release_s =
            std::clamp(extractFloat(val, "release_s", partial_.amp_env.release_s), 0.0f, 20.0f);
    } else if (key == "reverb") {
