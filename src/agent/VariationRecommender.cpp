...
std::vector<VariationSuggestion> VariationRecommender::suggest(const PatchStruct& current) {
    ...
    // Strategy 1: Temperature sweep — slightly more aggressive
    {
        ...
        if (!variants.empty()) {
            suggestions.push_back({
                "More Aggressive",
                "Boosted resonance, sharper filter, punchier envelope",
                variants[0],
                VariationEngine::Strategy::TemperatureSweep
            });
        }
    }
    ...
    // Strategy 2: Parameter perturbation — darker/warmer variant
    {
        ...
        suggestions.push_back({
            "Warmer / Darker",
            "Lowered cutoff, increased resonance, slower attack",
            darker,
    ...
    // Strategy 3: Morph toward bright/plucky
    {
        ...
        suggestions.push_back({
            "Brighter / Pluckier",
            "Higher cutoff, faster attack/decay, more saw wave",
            brighter,
            VariationEngine::Strategy::Perturbation
        });
    }
