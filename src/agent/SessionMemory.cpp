...
PatchVector SessionMemory::promptToVector(const std::string& prompt) noexcept {
    ...
    if (has("dark") || has("sub") || has("deep") || has("murky")) v[0] = 0.1f;
    else if (has("bright") || has("crisp") || has("sharp") || has("ice")) v[0] = 0.9f;
    else if (has("warm") || has("mellow") || has("soft")) v[0] = 0.4f;
    else v[0] = 0.5f;

    // [1] resonance: resonant/acid↑
    if (has("resonant") || has("acid") || has("squelch")) v[1] = 0.8f;
    else v[1] = 0.2f;

    // [2] attack: pad/slow↑ lead/pluck↓
    if (has("pad") || has("slow") || has("ambient") || has("evolving")) v[2] = 0.7f;
    else if (has("lead") || has("pluck") || has("stab") || has("snap")) v[2] = 0.05f;
    else v[2] = 0.3f;

    // [3] sustain: pad/sustained↑ bass/pluck↓
    if (has("pad") || has("sustained") || has("drone")) v[3] = 0.9f;
    else if (has("pluck") || has("stab") || has("short")) v[3] = 0.0f;
    else v[3] = 0.7f;

    // [4] lfo_depth: moving/evolving/wobble↑
    if (has("evolving") || has("moving") || has("wobble") || has("vibrato")) v[4] = 0.8f;
    else if (has("static") || has("still")) v[4] = 0.0f;
    else v[4] = 0.2f;

    // [5] reverb: ambient/wide/space↑ dry/bass↓
    if (has("ambient") || has("wide") || has("space") || has("reverb")) v[5] = 0.8f;
    else if (has("dry") || has("bass") || has("mono")) v[5] = 0.05f;
    else v[5] = 0.3f;
...
void SessionMemory::recordFeedback(FeedbackKind kind, const std::string& prompt,
                                    const PatchStruct& patch) {
...
PatchVector SessionMemory::computeParameterBias(const std::string& currentPrompt) const {
    ...
    for (const auto& ev : events_) {
        ...
        if (sim < kSimilarityThreshold) continue;
    ...
    if (totalWeight > 1e-9f) {
        for (float& b : bias) b /= totalWeight;
    }

    // Clamp to [-1, +1].
    for (float& b : bias) b = std::clamp(b, -1.0f, 1.0f);
