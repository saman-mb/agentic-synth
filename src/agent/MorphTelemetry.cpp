    ...
    for (size_t off = 0; off < msg.size(); off += 64) {
        ...
        for (int i = 0; i < 16; ++i) {
            w[i] = (static_cast<uint32_t>(msg[off + 4 * i]) << 24) |
                   ...
                   (static_cast<uint32_t>(msg[off + 4 * i + 2]) << 8) | (static_cast<uint32_t>(msg[off + 4 * i + 3]));
        }
        for (int i = 16; i < 64; ++i) {
...
void MorphTelemetry::log(const MorphEvent& ev) noexcept {
    try {
        ...
        case MorphEventKind::VariationPicked:
            ...
            break;
        case MorphEventKind::MacroTweaked:
            ss << ",\"macro_index\":" << ev.macro_index << ",\"value\":" << ev.value << ",\"dwell_ms\":" << ev.dwell_ms;
            break;
        case MorphEventKind::ABToggled:
            ss << ",\"from_slot\":" << ev.from_slot << ",\"to_slot\":" << ev.to_slot;
            break;
        case MorphEventKind::PresetCommitted:
            ss << ",\"name_length\":" << ev.name_length << ",\"prompt_hash\":\"" << jsonEscape(ev.prompt_hash) << "\"";
            break;
        case MorphEventKind::BounceToWav:
