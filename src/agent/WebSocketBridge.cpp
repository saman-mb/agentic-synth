...
std::array<uint8_t, 20> WebSocketBridge::sha1(const std::string& msg) {
    ...
    for (size_t blk = 0; blk < m.size(); blk += 64) {
        uint32_t w[80];
        for (int j = 0; j < 16; ++j)
            w[j] = (static_cast<uint32_t>(m[blk + j * 4])     << 24)
                 | (static_cast<uint32_t>(m[blk + j * 4 + 1]) << 16)
                 | (static_cast<uint32_t>(m[blk + j * 4 + 2]) <<  8)
                 |  static_cast<uint32_t>(m[blk + j * 4 + 3]);
        for (int j = 16; j < 80; ++j)
...
void WebSocketBridge::run() {
    ...
    while (!threadShouldExit()) {
        ...
        juce::Thread::launch([this, client, id]() {
...
void WebSocketBridge::handleClient(juce::StreamingSocket* sockRaw, int id) {
    ...
    if (!performHandshake(sock.get())) {
        ...
        clients_.erase(
            std::remove_if(clients_.begin(), clients_.end(),
                [id](const ClientEntry& e) { return e.id == id; }),
            clients_.end());
        return;
    }
    ...
    while (sock->isConnected()) {
        ...
        if (!readFrame(sock.get(), opcode, payload)) break;

        if (opcode == 0x8) break;               // connection close
...
bool WebSocketBridge::performHandshake(juce::StreamingSocket* sock) {
    ...
    char ch;
    while (sock->isConnected()) {
        if (sock->read(&ch, 1, true) != 1) return false;
        request += ch;
        if (request.size() >= 4 &&
    ...
    auto extractHeader = [&](const std::string& name) -> std::string {
        ...
        if (pos == std::string::npos) {
            ...
            for (char& c : needleLower) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
    ...
    return sock->write(response.c_str(), static_cast<int>(response.size()))
...
bool WebSocketBridge::readFrame(juce::StreamingSocket* sock,
                                uint8_t&              opcode,
                                std::vector<uint8_t>& payload) {
    uint8_t hdr[2];
    if (sock->read(hdr, 2, true) != 2) return false;
    ...
    uint8_t mask[4] = {};
    if (masked && sock->read(mask, 4, true) != 4) return false;

    payload.resize(static_cast<size_t>(paylen));
    size_t got = 0;
    while (got < static_cast<size_t>(paylen)) {
        int n = sock->read(payload.data() + got,
                           static_cast<int>(paylen - got), true);
        if (n <= 0) return false;
...
bool WebSocketBridge::writeFrame(juce::StreamingSocket* sock,
    ...
    return sock->write(frame.data(), static_cast<int>(frame.size()))
